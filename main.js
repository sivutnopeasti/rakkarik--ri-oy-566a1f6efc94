...
function getStatBarWidth(bar) {
  const valEl = bar.closest('.stat-item') && bar.closest('.stat-item').querySelector('.stat-value');
  const raw = valEl ? parseInt(valEl.textContent) : parseInt(bar.dataset.width);
  return isNaN(raw) ? 0 : raw;
}

function animateStatBar(bar) {
  bar.style.transition = 'width 1.2s ease-out';
  bar.style.width = getStatBarWidth(bar) + '%';
}
...
const statObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (!entry.isIntersecting) return;
    setTimeout(() => animateStatBar(entry.target), 200);
...
// Editorin live-päivitys: MutationObserver kuuntelee stat-value tekstin muutoksia
document.querySelectorAll('.stat-value').forEach(valEl => {
  new MutationObserver(() => {
    const bar = valEl.closest('.stat-item') && valEl.closest('.stat-item').querySelector('.stat-bar');
    if (bar) animateStatBar(bar);
